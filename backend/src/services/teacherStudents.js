...
function rosterRows(classId, organizationId) {
  return db.prepare(`SELECT s.id, s.first_name, s.last_name, s.grade, s.section,
    ...
    FROM school_students s JOIN class_enrollments ce ON ce.student_id=s.id WHERE ce.class_id=? ORDER BY s.first_name, s.last_name`).all(classId, classId, classId, classId, classId).map((row) => {
      ...
      return { id:row.id, name:`${row.first_name} ${row.last_name}`, className:`Class ${row.grade}-${row.section}`, assessmentCompletion, roadmapProgress:Math.round(row.roadmap_progress), weeklyGoalCompletion, careerExplorationStatus:row.careers_explored ? 'Explored' : 'Not started', lastActiveAt:row.last_active_at, lastActiveDays, attentionStatus:signals.length > 1 ? 'at_risk' : signals.length ? 'watch' : 'on_track', signals };
    });
}
